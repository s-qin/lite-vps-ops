...
write_state() {
  ...
  content=$(printf '{"schema_version":%s,"tool":"lite-vps-ops","version":"%s","profile":"%s","health_timer_policy":"%s","health_timer_policy_source":"%s","migration_from":"%s","resource_envelope":%s,"ssh_safety_status":"%s","ssh_reconnect_verified":%s,"ssh_rollback_guard":"%s","ssh_blackbox_verified":%s,"managed_hashes":%s}\n' \
    "$LVO_SCHEMA_VERSION" "$LVO_VERSION" "$PROFILE" "$HEALTH_TIMER_POLICY" "$HEALTH_TIMER_POLICY_SOURCE" "$MIGRATION_FROM" \
    "$(resource_envelope_state_json)" "${SSH_SAFETY_STATUS:-not-evaluated}" "${SSH_BLACKBOX_PASSED:-false}" "${SSH_GUARD_STATUS:-not-required}" "$SSH_BLACKBOX_PASSED" "$(managed_hashes_json)")
...
receipt_write() {
  ...
  printf '{"schema_version":%s,"tool":"lite-vps-ops","version":"%s","run_id":"%s","timestamp_utc":"%s","hostname":"%s","os":"%s","kernel":"%s","architecture":"%s","mode":"%s","profile":"%s","health_timer_policy":"%s","health_timer_policy_source":"%s","migration_from":"%s","resource_envelope":%s,"retention":{"keep":%s,"max_age_days":%s,"max_bytes":%s},"outcome":"%s","changed":%s,"package_action":"%s","swap_action":"%s","rollback":%s,"rollback_ok":%s,"manifest_sha256":"%s","reboot_required":%s,"ssh_safety":{"status":"%s","reconnect_verified":%s,"rollback_guard":"%s"},"ssh_blackbox_verified":%s,"checks":%s,"node_baseline_ready":%s}\n' \
    ...
    "${SSH_SAFETY_STATUS:-not-evaluated}" "${SSH_BLACKBOX_PASSED:-false}" "${SSH_GUARD_STATUS:-not-required}" \
  ...
  chmod 0600 "$file"
  {
    ...
    printf -- '- Changed: `%s`\n- Rollback: `%s` (ok: `%s`)\n- SSH safety: `%s` (reconnect: `%s`, guard: `%s`)\n- NODE_BASELINE_READY: `%s`\n\n' \
      "${TX_CHANGED:-0}" "${TX_ROLLBACK:-false}" "${TX_ROLLBACK_OK:-true}" "${SSH_SAFETY_STATUS:-not-evaluated}" \
      "${SSH_BLACKBOX_PASSED:-false}" "${SSH_GUARD_STATUS:-not-required}" "$ready"
